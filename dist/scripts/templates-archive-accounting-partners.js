...
/******/ 	var installedChunks = {
/******/ 		"templates-archive-accounting-partners": 0
/******/ 	};
/******/
/******/ 	var deferredModules = [];
/******/
...
/******/ 	deferredModules.push([27,"common-modules","vendor-core-js"]);
...
/***/ "./scripts/templates/archive/accounting-partners.js":
/*!**********************************************************!*\
  !*** ./scripts/templates/archive/accounting-partners.js ***!
  \**********************************************************/
/*! no static exports found */
...
const amGetLocations = () => {
  if (!window.sessionStorage.getItem('accounting_partners_locations')) {
    ...
    if (apTaxonomies !== null) {
      ...
      window.sessionStorage.setItem('accounting_partners_locations', JSON.stringify(locations[0].terms));
    }
  }
};
...
const amTaxonomies = () => {
  if (!window.sessionStorage.getItem('accounting_partners_taxonomies')) {
    $.ajax({
      ...
      success(response) {
        window.sessionStorage.setItem('accounting_partners_taxonomies', JSON.stringify(response.body_response));
        amTaxonomiesObj = response.body_response;
      }

    });
  } else {
    amTaxonomiesObj = JSON.parse(window.sessionStorage.getItem('accounting_partners_taxonomies'));
  }
...
const checkAccountingPartnersUrl = amTaxObj => {
  ...
  if (countSlash >= 4 && urlSplit[2] !== 'page') {
    ...
    if (location[0].type_code === 3) {
      ...
      obj.accounting_partners_location = [{
        slug: country[0].parent_slug,
        name: country[0].parent_name
      }, {
        slug: location[0].parent_slug,
        name: location[0].parent_name
      }, {
        slug: selectedDataSlug,
        name: selectedVal
      }];
...
const closeLocation = obj => {
  ...
  if (locationInput !== null && !locationInput.classList.contains('clicked')) {
    var _locationInput$classL;

    bLocation.style.display = 'block';
    bLocation.classList.add('opc-trans');
    (_locationInput$classL = locationInput.classList) === null || _locationInput$classL === void 0 ? void 0 : _locationInput$classL.remove('active');
    setTimeout(() => {
...
const buttonEnter = () => {
  const amButtons = document.querySelectorAll('.accounting-partners-filter__button:not(.accounting-partners-filter__button_location)');
  ...
  const funcBackHover = e => {
    ...
    document.querySelector('.accounting-partners-filter__back-hover').addEventListener('click', () => {
      buttonOut();
    });
    ...
    if (e.target.classList.contains('active')) {
      ...
      amButtons.forEach(ele => {
        ...
        (_ele$classList = ele.classList) === null || _ele$classList === void 0 ? void 0 : _ele$classList.remove('active');
      });
...
const buttonOut = () => {
  ...
  const amButtons = document.querySelectorAll('.accounting-partners-filter__button:not(.accounting-partners-filter__button_location)');
  bgHover === null || bgHover === void 0 ? void 0 : bgHover.classList.add('accounting-partners-filter__back-hover-out');
  setTimeout(() => {
    bgHover === null || bgHover === void 0 ? void 0 : bgHover.classList.remove('accounting-partners-filter__back-hover-out');
    bgHover === null || bgHover === void 0 ? void 0 : bgHover.remove();
  }, 500);
  amButtons.forEach(ambutton => {
...
const activesOut = () => {
  ...
  if (window.innerWidth >= 1024) {
    acContainer === null || acContainer === void 0 ? void 0 : acContainer.addEventListener('mouseleave', () => {
      buttonOut();
      closeLocation();
    });
    acButtonLocation === null || acButtonLocation === void 0 ? void 0 : acButtonLocation.addEventListener('mouseenter', () => {
      buttonOut();
    });
  } else {
    acButtonLocation === null || acButtonLocation === void 0 ? void 0 : acButtonLocation.addEventListener('click', () => {
      buttonOut();
    });
  }
};
...
const showLocation = e => {
  const obj = e.target;
  const locationInput = document.querySelector('.accounting-partners-filter__location-input');
  locationInput === null || locationInput === void 0 ? void 0 : locationInput.addEventListener('click', function () {
...
const locationButton = () => {
  ...
  if (window.innerWidth >= 768 && !isMobileDevice()) {
    bLocation === null || bLocation === void 0 ? void 0 : bLocation.addEventListener('mouseover', showLocation);
  } else {
    bLocation === null || bLocation === void 0 ? void 0 : bLocation.addEventListener('click', showLocation);
  }
};
/* global navigator */
  ...
  if (inp !== null) {
    inp.addEventListener('input', function () {
      ...
      for (i = 0; i < arr.length; i++) {
        if (arr[i].name.substr(0, val.length).toUpperCase() === val.toUpperCase() || arr[i].name_slug.substr(0, val.length).toUpperCase() === val.toUpperCase()) {
          b = document.createElement('DIV');
          b.innerHTML = "\n\t\t\t\t\t\t\t".concat(arr[i].name.substr(0, val.length)).concat(arr[i].name.substr(val.length)).concat(arr[i].parent_slug !== undefined ? ', ' + arr[i].parent_slug.substr(0, val.length).toUpperCase() : '', " \n\t\t\t\t\t\t<input type='hidden' value='").concat(arr[i].name, "' data-slug='").concat(arr[i].name_slug, "'></input>");
          b.addEventListener('click', function () {
            ...
            window.sessionStorage.setItem('accounting_partners_filter', JSON.stringify(aFilterJson));
            closeAllLists();
            ...
            if (window.innerWidth >= 1024) {
              inp.value = '';
              document.querySelector('.accounting-partners-filter__button_location').style.display = 'block';
      ...
      if (Array.isArray(aFilterData[property])) {
        aFilterData[property].map(item => {
          if ('accounting_partners_location' !== property) {
            document.querySelector("#".concat(property, " [data-slug=").concat(item.slug, "]")).classList.add('active');
            document.querySelector("#".concat(property)).previousElementSibling.classList.add('selected');
          }

          return false;
        });
      } else {
        document.querySelector("#".concat(property, " [data-slug=").concat(aFilterData[property], "]")).classList.add('active');
      }
    }
  }
...
const addNumberOfSelectedPerTaxnomy = amButtons => {
  if (window.innerWidth < 768) {
    amButtons.forEach(button => {
      const buttonsActive = button.nextElementSibling.querySelectorAll('.accounting-partners-filter__term.active');
...
const filterTermButtonEvent = () => {
  const aFilterTermButton = document.querySelectorAll('.accounting-partners-filter__term:not(.disabled)');

  for (const button of aFilterTermButton) {
    button.addEventListener('click', function () {
      const termSlug = this.getAttribute('data-slug');
      const termName = this.querySelector('.accounting-partners-filter__term-name').textContent;
...
const breadcrumbsUnit = (tax, name, slug) => {
  return "<button class=\"accounting-partners-filter-elements__button\" data-slug=\"".concat(slug, "\" data-tax-slug=\"").concat(tax.name_slug, "\"><span>").concat(tax.name, ": </span>").concat(name, "</button>");
};

const generateBreadcrumbs = obj => {
  const aFilterElements = document.querySelector('.accounting-partners-filter-elements');
  amTaxonomiesObj = JSON.parse(window.sessionStorage.getItem('accounting_partners_taxonomies'));
  const locations = JSON.parse(window.sessionStorage.getItem('accounting_partners_locations'));
  ...
  for (const property in obj) {
    if (property === 'accounting_partners_location') {
      obj[property].map(item => {
        var _amTaxonomiesObj;

        const tax = (_amTaxonomiesObj = amTaxonomiesObj) === null || _amTaxonomiesObj === void 0 ? void 0 : _amTaxonomiesObj.filter(term => {
          return term.name_slug === property;
        });
        const location = locations === null || locations === void 0 ? void 0 : locations.filter(term => {
        ...
        if (location.length === 0) {
          buttonsFiltered += breadcrumbsUnit({
            name: 'Location',
            name_slug: 'accounting_partners_location'
          }, item.name, item.slug);
        } else {
          buttonsFiltered += breadcrumbsUnit({
            name: location[0].type,
            name_slug: tax[0].name_slug
          }, item.name, item.slug);
        }
      ...
      obj[property].map(item => {
        var _amTaxonomiesObj2;

        const tax = (_amTaxonomiesObj2 = amTaxonomiesObj) === null || _amTaxonomiesObj2 === void 0 ? void 0 : _amTaxonomiesObj2.filter(term => {
          return term.name_slug === property;
        });
  ...
  if (flag) {
    buttonsFiltered += '<button class="accounting-partners-filter-elements__clear-all">Clear All</button>';
...
const clearAllFilter = button => {
  if (button === undefined) {
    ...
    const objFilter = JSON.parse(window.sessionStorage.getItem('accounting_partners_filter'));
    const tax = button.getAttribute('data-tax-slug');
    const term = button.getAttribute('data-slug');
    let termsData = '';

    if (tax === 'accounting_partners_location') {
      const mlButtons = document.querySelectorAll('.accounting-partners-filter-elements__button[data-tax-slug="accounting_partners_location"]');
      let mlPosition = 0;
      let mlButtonSlugs = [];
      ...
      if (mlPosition === 2 && mlButtons.length === 3) {
        ...
        termsData = objFilter[tax].filter(item => {
          return item.slug !== mlButtonSlugs[0] && item.slug !== mlButtonSlugs[1] && item.slug !== mlButtonSlugs[2];
        });
...
const getAllTermsSelected = () => {
  ...
  if (objData !== null) {
    if (objData.accounting_partners_industry !== undefined || Array.isArray(objData.accounting_partners_industry) && objData.accounting_partners_industry.length) {
      allTermsSelected.push(...ArrObjectToArray(objData.accounting_partners_industry));
    }

    if (objData.accounting_partners_language !== undefined || Array.isArray(objData.accounting_partners_language) && objData.accounting_partners_language.length) {
      allTermsSelected.push(...ArrObjectToArray(objData.accounting_partners_language));
    }

    if (objData.accounting_partners_location !== undefined || Array.isArray(objData.accounting_partners_location) && objData.accounting_partners_location.length) {
      allTermsSelected.push(...ArrObjectToArray(objData.accounting_partners_location.slice(-1)));
    }

    if (objData.accounting_partners_service !== undefined || Array.isArray(objData.accounting_partners_service) && objData.accounting_partners_service.length) {
...
const backFilter = (page, animate = false, clicked = null) => {
  ...
  $.ajax({
    ...
    success(response) {
      ...
      filteredPaginator();
      cardHover();
      let newUrl = "".concat(window.location.href, "/page/").concat(page);

      if (window.location.href.includes('/page/')) {
        newUrl = "".concat(window.location.href.split('/page/')[0], "/page/").concat(page);
      }

      if (parseInt(page) === 1) {
        newUrl = "".concat(window.location.href.split('/page/')[0]);
      }
      ...
      if (animate) {
        ...
        scrollAnimate(document.scrollingElement || document.documentElement, 'scrollTop', '', document.scrollingElement.scrollTop || document.documentElement.scrollTop, target.offsetTop, 600, true);
      }
    }
    ...
    success(response) {
      ...
      if (Object.keys(listTermsCount).length !== 0) {
        parentContainer.forEach(pContainer => {
          const numbers = document.querySelectorAll(".accounting-partners-filter__term:not(#".concat(pContainer, " .accounting-partners-filter__term)"));

          for (let i = 0; i < numbers.length; i++) {
            numbers[i].classList.add('disabled');
            numbers[i].querySelector(".accounting-partners-filter__term_gray").innerHTML = "(0)";
          }

          for (const term in listTermsCount) {
            const termButton = document.querySelector(".accounting-partners-filter__term[data-slug=\"".concat(term, "\"]:not(#").concat(pContainer, " .accounting-partners-filter__term[data-slug=\"").concat(term, "\"])"));
            ...
            if (termButton !== null) {
              ...
              if (number !== null) {
                number.innerHTML = "(".concat(listTermsCount[term], ")");
              }
            }
          }
        });
...
const resetTermButtonsCount = () => {
  const initTermsCount = JSON.parse(window.sessionStorage.getItem('accounting_partners_init_term_count'));

  for (let i = 0; i < initTermsCount.length; i++) {
    const termButton = document.querySelector(".accounting-partners-filter__term[data-slug=\"".concat(initTermsCount[i].slug, "\"]"));
...
const saveTermCount = () => {
  if (!window.sessionStorage.getItem('accounting_partners_init_term_count')) {
    ...
    for (let i = 0; i < numbers.length; i++) {
      ...
      const number = numbers[i].querySelector('.accounting-partners-filter__term_gray').innerHTML;
      newObj.slug = slug;
      newObj.number = number;
      arr.push(newObj);
    }

    window.sessionStorage.setItem('accounting_partners_init_term_count', JSON.stringify(arr));
  }
};
...
const toggleSingleTerms = () => {
  if (window.innerWidth >= 768) {
    ...
    const singlesContainerWidth = (_document$querySelect = document.querySelector('.accounting-partners-content__description')) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.offsetWidth;
    singles.forEach(single => {
      const terms = single.querySelectorAll('.accounting-partners-content__term');
      let termsWidth = parseInt(41);
      let termsHiddenHtml = '<button class="show-terms">+ more</button><div class="terms-hidden">';
      let termsHtml = '';
      ...
      mores.forEach(more => {
        more.addEventListener('click', function () {
          ...
          (_this$nextElementSibl = this.nextElementSibling) === null || _this$nextElementSibl === void 0 ? void 0 : _this$nextElementSibl.classList.add('d-flex');
          this.remove();
        });
      });
    });
...
const mobileSerachTrigger = () => {
  if (window.innerWidth < 768) {
    const buttonTrigger = document.querySelector('.accounting-partners-filter__trigger-button');
    const body = document.querySelector('body');
    buttonTrigger.addEventListener('click', () => {
      ...
      filterSelect.classList.toggle('active');
      buttonOut();
      const filterObj = JSON.parse(window.sessionStorage.getItem('accounting_partners_filter'));
...
/***/ "./styles/templates/archive/accounting-partners.scss":
/*!***********************************************************!*\
  !*** ./styles/templates/archive/accounting-partners.scss ***!
  \***********************************************************/
/*! no static exports found */
...
/***/ 27:
/*!********************************************************************************************************************!*\
  !*** multi ./scripts/templates/archive/accounting-partners.js ./styles/templates/archive/accounting-partners.scss ***!
  \********************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

__webpack_require__(/*! /Users/xue.ding/freshpress-website/themes/freshpress/assets/scripts/templates/archive/accounting-partners.js */"./scripts/templates/archive/accounting-partners.js");
module.exports = __webpack_require__(/*! /Users/xue.ding/freshpress-website/themes/freshpress/assets/styles/templates/archive/accounting-partners.scss */"./styles/templates/archive/accounting-partners.scss");
...
/******/ });
//# sourceMappingURL=templates-archive-accounting-partners.js.map