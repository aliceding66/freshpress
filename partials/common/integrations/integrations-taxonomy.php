<?php
/**
 * Taxonomy include for Integration Categories.
 *
...
<div class="integrations-taxonomy">
	...
	>
		...
		<div class="integrations-taxonomy__left-column col col-12 col-lg-3 mx-auto p-0 pr-lg-5">
			...
			<div
				...
				<div class="integrations-taxonomy__navigation-inner-wrapper pb-3 d-md-flex row wrap px-3">
					<a href="/integrations" class="integrations-taxonomy__home-link pl-3 mb-5 d-none d-lg-block text-decoration-none">Integrations Home</a>
					...
					<div class="integrations-taxonomy__navigation-collections-wrapper">
						...
						<?php foreach ( $collections_list as $collections_item ) : ?>
							<?php if ( 'featured' !== $collections_item->slug ) : ?>
								<?php
								$current_col_class = '';
								if ( $queried_object->slug === $collections_item->slug ) {
									$current_col_class = 'current';
								}
								?>
								<li class="py-1 px-2 mb-0 <?= esc_attr( $current_col_class ); ?>">
					...
					<div class="integrations-taxonomy__navigation-categories-wrapper pl-md-5 pl-lg-0">
						...
						<ul class="integrations-taxonomy__navigation-list integrations-taxonomy__navigation-list_categories mb-5 list-unstyled">
							<li class="py-1 px-2 mb-0 integrations-taxonomy__navigation-item"><a href="/integrations/pages" class="text-decoration-none d-block mb-0">View All Integrations</a></li>
							<?php foreach ( $categories_list as $categories_item ) : ?>
								<?php
								$current_cat_class = '';
								if ( $queried_object->slug === $categories_item->slug ) {
		...
		<div class="integrations-taxonomy__right-column col col-12 col-lg-9 mx-auto px-3">
			...
			<h2 class="mb-4"><?= esc_html( $queried_object->name ); ?></h2>
				...
				<?php foreach ( $integrations as $key => $integration ) : ?>
					...
					echo fp_render_blocks(
						[
							...
							'attrs' => [
								'data'      => [
									'integration_card_id' => $integration->ID,
								],
					...
					<?php
					if (
						( $integrations_count >= 6 ) && ( 4 === $key ) ||
						( $integrations_count < 6 ) && ( ( $integrations_count - 1 ) === $key )
					) :
						?>
						<!-- Ad -->
